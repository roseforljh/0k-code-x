"""
ChatGPT 批量自动注册工具 (并发版) - CF Email Worker 版
依赖: pip install curl_cffi
功能: 使用 CF Email Worker 临时邮箱，并发自动注册 ChatGPT 账号，自动获取 OTP 验证码
"""
...
def _load_config():
    ...
    config["cfemail_url"] = os.environ.get("CFEMAIL_URL", config["cfemail_url"])
    config["cfemail_password"] = os.environ.get("CFEMAIL_PASSWORD", config["cfemail_password"])
...
EMAIL_BACKEND = "cfemail"
CFEMAIL_URL = _CONFIG.get("cfemail_url", "")
CFEMAIL_PASSWORD = _CONFIG.get("cfemail_password", "")
...
def create_temp_email():
    ...
    return create_cfemail_mailbox()


def _build_cfemail_basic_auth_header():
    import base64
    payload = ":".join(["admin", CFEMAIL_PASSWORD])
    return "Basic " + base64.b64encode(payload.encode()).decode()

def _create_cfemail_session():
    """创建 CF Email Worker 请求会话"""
    session = curl_requests.Session()
    session.headers.update({
        "User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36",
        "Accept": "application/json",
        "Content-Type": "application/json",
        "Authorization": _build_cfemail_basic_auth_header(),
    })
    return session


def create_cfemail_mailbox():
    """通过 CF Email Worker API 创建邮箱"""
    if not CFEMAIL_URL or not CFEMAIL_PASSWORD:
        raise Exception("CFEmail 配置不完整")

    session = _create_cfemail_session()
    api_base = CFEMAIL_URL.rstrip("/")
    ...
    try:
        ...
        # 只使用能收到邮件的域名
        working_domains = ['5945946.xyz', 'a1ways.nyc.mn', 'a1ways.de', 'kuz7.com']
        enabled_domains = [d for d in domains if d.get("enabled") and d.get("domain") in working_domains]
        if not enabled_domains:
            enabled_domains = [d for d in domains if d.get("domain") in working_domains]
        if not enabled_domains:
            enabled_domains = domains  # 没有就fallback到全部

        # 随机选择一个域名
        domain_info = random.choice(enabled_domains)
        ...
        email = f"{email_local}@{domain_name}"

        payload = {"address": email_local, "name": email_local, "domain_id": domain_id}
        res = session.post(
            f"{api_base}/api/mailboxes",
            json=payload,
            timeout=15,
            impersonate="chrome131"
        )
        ...
        return email, "", email  # 返回 email, password(空), mail_token(即 email)

    except Exception as e:
        raise Exception(f"CFEmail 创建邮箱失败: {e}")


def _fetch_emails_cfemail(email: str):
    """从 CF Email Worker 获取邮件列表"""
    try:
        api_base = CFEMAIL_URL.rstrip("/")
        session = _create_cfemail_session()

        res = session.get(
            f"{api_base}/api/emails?address={email}",
            timeout=15,
            impersonate="chrome131"
        )
        ...
        print(f"[CFEmail] 获取邮件列表失败: {e}")
    return []


def _fetch_email_detail_cfemail(email: str, msg_id: str):
    """从 CF Email Worker 获取邮件详情"""
    try:
        api_base = CFEMAIL_URL.rstrip("/")
        session = _create_cfemail_session()

        # CF Email API 返回的邮件数据已包含详情，这里直接返回
        # 需要的话可以调用单个邮件详情 API
        res = session.get(
            f"{api_base}/api/emails?address={email}",
            timeout=15,
        ...
        if res.status_code == 200:
            ...
            if data.get("success"):
                ...
                for em in emails:
                    if str(em.get("id")) == str(msg_id):
                        # 组合 HTML 和文本内容
                        content = em.get("body_html") or em.get("body_text") or ""
                        return {"html": em.get("body_html"), "text": em.get("body_text"), "content": content}
    except Exception as e:
        print(f"[CFEmail] 获取邮件详情失败: {e}")
    return None


def _extract_verification_code(email_content: str, subject: str = None):
    """从邮件内容提取 6 位验证码"""
    # 先从主题中提取（格式: Your ChatGPT code is XXXXXX）
    if subject:
        match = re.search(r"code is (\d{6})", subject, re.IGNORECASE)
        if match:
...
def wait_for_verification_email(mail_token: str, timeout: int = 120):
    ...
    while time.time() - start_time < timeout:
        messages = _fetch_emails_cfemail(mail_token)
        ...
        if messages and len(messages) > 0:
            ...
            if msg_id:
                detail = _fetch_email_detail_cfemail(mail_token, msg_id)
...
class ChatGPTRegister:
    ...
    def _create_cfemail_session(self):
        """创建 CF Email Worker 请求会话"""
        session = curl_requests.Session()
        session.headers.update({
            "User-Agent": self.ua,
    ...
    def _create_cfemail_mailbox(self):
        """通过 CF Email Worker API 创建邮箱"""
        if not CFEMAIL_URL or not CFEMAIL_PASSWORD:
            raise Exception("CFEmail 配置不完整")

        session = self._create_cfemail_session()
        api_base = CFEMAIL_URL.rstrip("/")
    ...
    def create_temp_email(self):
        """创建临时邮箱，返回 (email, password, mail_token)"""
        return self._create_cfemail_mailbox()

    def _fetch_emails_cfemail(self, email: str):
        """从 CF Email Worker 获取邮件列表"""
        try:
            api_base = CFEMAIL_URL.rstrip("/")
            session = self._create_cfemail_session()
            ...
            self._print(f"[CFEmail] 获取邮件列表失败: {e}")
        return []

    def _fetch_email_detail_cfemail(self, email: str, msg_id: str):
        """从 CF Email Worker 获取邮件详情"""
        try:
            api_base = CFEMAIL_URL.rstrip("/")
            session = self._create_cfemail_session()

            # CF Email API 返回的邮件数据已包含详情
            res = session.get(
                f"{api_base}/api/emails?address={email}",
                timeout=15,
                impersonate=self.impersonate
            ...
            self._print(f"[CFEmail] 获取邮件详情失败: {e}")
        return None

    def _extract_verification_code(self, email_content: str, subject: str = None):
        """从邮件内容提取 6 位验证码"""
        # 先从主题中提取（格式: Your ChatGPT code is XXXXXX）
        if subject:
            match = re.search(r"code is (\d{6})", subject, re.IGNORECASE)
...
def run_batch(total_accounts: int = 3, output_file=None,
    ...
    if output_file is None:
        output_file = os.path.join(_OUTPUT_DIR, "registered_accounts.txt")
    ...
    backend_name = "CFEmail"
    actual_workers = min(max_workers, total_accounts)
    print(f"\n{'#'*60}")
    print(f"  ChatGPT 批量自动注册")
    print(f"  邮箱后端: {backend_name}")
    ...
    if ENABLE_OAUTH:
        ...
        print(f"  Token输出: {TOKEN_JSON_DIR}/")
